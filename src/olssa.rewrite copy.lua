...
do
	...
	local __olssa_identifier = (function()
		-- Generates a unique OLSSA-session identifier, which can be used to string match logs (and hide them if hooking LogService)
		-- __olssa_identifier --> "%451676bcada921d7%"
		return string.format("%%%04x%04x%04x%04x%%", 
				
				math.random(0, 0xFFFF), math.random(0, 0xFFFF), math.random(0, 0xFFFF), math.random(0, 0xFFFF)
			)
	end)()

	-- Set memory category
	debug.setmemorycategory(string.format("%s - OLSSA %s %s", script.Name, __olssa_configuration.REVISION, __olssa_identifier)
	...
	-- TO:DO; Hook logservice etc. to hide OLSSA logs
